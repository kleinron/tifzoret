...
describe('generatePuzzle uniqueness', () => {
  ...
  it('fails in Hebrew when the 4×4 blocks cannot fit', () => {
    ...
    expect(result.errorHe).toContain('לכל היותר 1')
  })

  it('fails in Hebrew when two pictures cannot keep a gap on 8×8', () => {
    const result = generatePuzzle({
      size: 8,
      userWords: ['שמש'],
      directions: [...DEFAULT_DIRECTION_IDS],
    ...
    expect(result.ok).toBe(false)
    if (result.ok) throw new Error('expected failure')
    expect(result.errorHe).toBe(imagePlacementErrorHe(2, 8))
    expect(result.errorHe).toContain('בלי מגע')
  })
