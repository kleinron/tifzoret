...
5. בחרו גודל רשת (8–20), **תמונות על הלוח** וגודל גופן (ברירת מחדל 18pt).
   ...
   - כל תמונה תופסת ריבוע **4×4** בלי אותיות. משתי תמונות ומעלה חייבת להישאר ביניהן לפחות משבצת ריקה אחת, בלי מגע בצלע או בפינה. המקסימום הוא כמה ריבועים כאלה נכנסים עם הרווח (למשל 4 בלוח 12×12, תמונה אחת בלוח 8×8). הקטנת הלוח חותכת את המספר אוטומטית. אם מבקשים יותר תמונות ממה שנכנס, היצירה נכשלת בעברית.
...
5. Grid size 8–20. **Pictures on the board** default to **1** (0 turns them off). Font size defaults to **18pt** (Assistant / Heebo).
   - Each picture occupies a **4×4** block that letters cannot use. From two pictures up, blocks keep at least one empty cell between them, including diagonally (they may not share an edge or a corner). The maximum is how many such blocks fit with that gap (4 on 12×12, 1 on 8×8). Shrinking the grid clamps the count. Asking for more pictures than fit fails with a clear Hebrew message.
