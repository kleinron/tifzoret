...
4. אפשרויות:
   - **מילוי אוטומטי לגיל ~10** — ביצירת תפזורת משלים מילים ידידותיות לילדים מקובץ מובנה (`src/data/kidWords.ts`), בלי רשת. הפעלה או כיבוי של האפשרות יוצרים תפזורת מחדש.
...
5. בחרו גודל רשת (8–20), **תמונות על הלוח** וגודל גופן (ברירת מחדל 18pt).
   ...
   - **«ערבב מחדש» משאיר את התמונות שכבר על הלוח** (אותו ציור, אותו מקום) ומערבב רק את האותיות. אם עדיין אין תמונות, הערבוב מתנהג כמו קודם לגבי המילים ומגריל תמונות לפי המספר שנבחר. «ערבב מחדש» לא מגריל מחדש את מילות גיל 10 שכבר נוספו.
   - אין לחצן «צור תפזורת». כל שינוי בהגדרות (כיוונים, מחסן המילים, מילוי גיל 10, אותיות סופיות, גודל הלוח, מספר התמונות) יוצר תפזורת. סליידר של גודל הלוח או של מספר התמונות יוצר תפזורת רק כשעוזבים אותו, לא תוך כדי גרירה. גם פתיחת הדף וקישור שיתוף יוצרים לוח בלי לחיצה. גודל הגופן וטיוטת המילים שטרם נוספה לא יוצרים מחדש. תמונות קיימות נשארות כל עוד הן נכנסות בלוח ובמספר שנבחר; אם הלוח קטן מדי או שהמספר ירד, תמונות שלא נכנסות יורדות והשאר נשמר.
...
4. Optional:
   - **Automatic age-~10 fill** — when a puzzle is built, extra kid-friendly words from the bundled corpus (offline). Turning the option on or off rebuilds the puzzle.
...
5. Grid size 8–20. **Pictures on the board** default to **1** (0 turns them off). Font size defaults to **18pt** (Assistant / Heebo).
   ...
   - **Reshuffle keeps pictures already on the board** (same drawing, same cells) and only re-places letters. With no pictures yet, reshuffle still follows the previous word rules and rolls pictures from the chosen count. Reshuffle does not draw a new age-10 word set.
   - There is no Generate button. Any puzzle setting change (directions, word bank, age-10 fill, no final letters, board size, picture count) builds a puzzle. The board-size and picture-count sliders build only when the drag is released, not on each intermediate value. Opening the page and opening a share link do too. Font size and the uncommitted word draft do not. Existing pictures stay when they still fit the board and the chosen count; a board that is too small, or a lower count, drops the pictures that no longer fit and keeps the rest.
