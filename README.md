...
5. בחרו גודל רשת (8–20), **תמונות על הלוח** וגודל גופן (ברירת מחדל 18pt).
   ...
   - **«צור תפזורת» מגריל ציורים חדשים.** כל שינוי בהגדרות (כיוונים, מחסן המילים, מילוי גיל 10, אותיות סופיות, גודל הלוח, מספר התמונות) יוצר תפזורת מיד, בלי לחיצה נוספת. גודל הגופן וטיוטת המילים שטרם נוספה לא יוצרים מחדש. תמונות קיימות נשארות כל עוד הן נכנסות בלוח ובמספר שנבחר; אם הלוח קטן מדי או שהמספר ירד, תמונות שלא נכנסות יורדות והשאר נשמר.
6. **צור תפזורת** או **ערבב מחדש**, או פשוט שנו הגדרה. כל מילת מחסן מופיעה **פעם אחת בלבד** בכיוונים הפעילים; משבצות התמונה חסומות ולא נספרות כאות. אם נוצרת הופעה מקרית, המחולל מערבב שוב.
...
5. Grid size 8–20. **Pictures on the board** default to **1** (0 turns them off). Font size defaults to **18pt** (Assistant / Heebo).
   ...
   - **Generate rolls new drawings.** Any puzzle setting change (directions, word bank, age-10 fill, no final letters, board size, picture count) builds a puzzle immediately, without a separate click. Font size and the uncommitted word draft do not. Existing pictures stay when they still fit the board and the chosen count; a board that is too small, or a lower count, drops the pictures that no longer fit and keeps the rest.
6. **Generate** / **Reshuffle**, or just change a setting. Each word in the store is placed **exactly once**. Image cells are blocked: they are not filled with letters, and a word span that enters one does not count. After random fill, the generator verifies there is no accidental second copy along enabled directions and retries until that holds (or fails clearly, in Hebrew, including when the pictures themselves cannot be placed).
