...
5. בחרו גודל רשת (8–20), **תמונות על הלוח** וגודל גופן (ברירת מחדל 18pt).
   ...
   - כל תמונה תופסת ריבוע **4×4** בלי אותיות. המקסימום הוא כמה ריבועים כאלה נכנסים בלי חפיפה (למשל 9 בלוח 12×12, 4 בלוח 8×8). הקטנת הלוח חותכת את המספר אוטומטית.
...
5. Grid size 8–20. **Pictures on the board** default to **1** (0 turns them off). Font size defaults to **18pt** (Assistant / Heebo).
   - Each picture occupies a **4×4** block that letters cannot use. The maximum is how many non-overlapping 4×4 blocks fit (9 on 12×12, 4 on 8×8). Shrinking the grid clamps the count.
