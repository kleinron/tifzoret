...
5. בחרו גודל רשת (8–20), **תמונות על הלוח** וגודל גופן (ברירת מחדל 18pt).
   ...
   - אין לחצן «צור תפזורת». כל שינוי בהגדרות (כיוונים, מחסן המילים, מילוי גיל 10, אותיות סופיות, גודל הלוח, מספר התמונות) יוצר תפזורת מיד. גם פתיחת הדף וקישור שיתוף יוצרים לוח בלי לחיצה. גודל הגופן וטיוטת המילים שטרם נוספה לא יוצרים מחדש. תמונות קיימות נשארות כל עוד הן נכנסות בלוח ובמספר שנבחר; אם הלוח קטן מדי או שהמספר ירד, תמונות שלא נכנסות יורדות והשאר נשמר.
...
5. Grid size 8–20. **Pictures on the board** default to **1** (0 turns them off). Font size defaults to **18pt** (Assistant / Heebo).
   ...
   - There is no Generate button. Any puzzle setting change (directions, word bank, age-10 fill, no final letters, board size, picture count) builds a puzzle immediately. Opening the page and opening a share link do too. Font size and the uncommitted word draft do not. Existing pictures stay when they still fit the board and the chosen count; a board that is too small, or a lower count, drops the pictures that no longer fit and keeps the rest.
